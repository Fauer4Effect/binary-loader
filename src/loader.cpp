...
// TODO fix the vscode settings so this gets included corrects
// TODO Add a cmakelists
#include "loader.hpp"
...
/**
 * @brief Parse a binary file specified by filename and load it into the
 * Binary object given to it.
 *
 * @param fname name of binary to load
 * @param bin Binary object to load into
 ...
 *
 * @param bin Binary object to free
 */
...
static bfd *open_bfd(std::string &fname) {
    ...
    // error before detecting the format and then neglect to unset it one
    // the format has been detected. We unset it manually to prevent
    // problems.
    bfd_set_error(bfd_error_no_error);

    if (bfd_get_flavour(bfd_h) == bfd_target_unknown_flavour) {
        fprintf(stderr, "unrecognized format for binary '%s' (%s)'n", fname.c_str(), bfd_errmsg(bfd_get_error()));
        return NULL;
    }
...
static int load_binary_bfd(std::string &fname, Binary *bin,
    ...
    switch (bfd_info->mach) {
        ...
        case bfd_mach_x86_64:
            ...
            break;
        default:
            fprintf(stderr, "unsupported architecutre (%s)\n",
                    bfd_info->printable_name);
            goto fail;
    }

    // symbol handling is best-effort only (they may not be present)
